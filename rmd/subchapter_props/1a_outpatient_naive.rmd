...
write_csv(df_mitigation_naive, here("csv", "df_mitigation_naive.csv"))
```
...
write_csv(df_mitigation_naive_without_first_quarters, here("csv", "df_mitigation_naive_without_first_quarters.csv"))
...
### Plot
```{r}
df_mitigation_naive <- read_csv(here("csv", "df_mitigation_naive.csv"))
df_mitigation_naive_without_first_quarters <- read_csv(here("csv", "df_mitigation_naive_without_first_quarters.csv"))
```
...
```{r}
df_mitigation_naive_without_first_quarters_changes <- df_mitigation_naive_without_first_quarters_p_values %>% 
  mutate(
    before = percent(prop, accuracy = .1),
    after = percent(prop + estimate, accuracy = .1),
    difference_in_percentage_points = percent(estimate, accuracy = .1),
    ratio_in_percent = percent(estimate/prop, accuracy = .1)) %>% 
...
```{r}
write.csv(df_mitigation_naive_without_first_quarters_changes, here("tables", "subchapter_prop_changes.csv"))
```
