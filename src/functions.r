...
add_patient_and_clinic_uid <- function(df,
                                       clinic_id_col,
                                       patient_id_col) {
...
visits_to_processed_treatment_courses <- function(df) {
  df_out <- df %>%
...
join_and_calculate_diagnoses_per_open_sequence <- function(df1, df2) {
  ...
  df_out <- df_out %>%
    rowwise() %>%
    mutate(
      diag_per_active = poisson.test(unique_diagnoses_3, open_sequences)$estimate,
