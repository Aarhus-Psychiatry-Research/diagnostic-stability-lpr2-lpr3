...
---
```{r}
source(here::here("subchapter_props", "0_setup_env.r"))
```

# Run 1a_outpatient_naive.rmd first!

## Outpatient, construct ambulatory sequences
```{r}
source(here::here("src", "functions.r"))
df_constructed_outpatient_series <- df_outpatient_for_subchapter %>% 
  mutate(constructed_id = paste0(dw_ek_borger, clinic))

# TODO: 
# 0. Generate a unique ID for each "sequence" (patient + clinic)
# 1. Process each unique entity of outpatient-clinic + patient-identifier uniquely
# 2. Split sequences if there are gaps > n months

```

### Constructed series, final visit
```{r}
...
```
### Constructed series, most severe visit
```{r}
