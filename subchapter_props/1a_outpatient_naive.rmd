...
```{r}
df_outpatient_with_periods <- df_raw_outpatient %>% 
  filter(ambbesoeg == 1) %>% 
  add_LPR23_quarter_column()

log_time()
```
## Outpatient, naive useage of kontakt and forløbsansvar

Reduce complexity
```{r}
df_outpatient_for_subchapter <- df_outpatient_with_periods %>% 
  select(dw_ek_borger,
         datotid_start,
...
Calculate which proportion of patients have gotten a diagnosis within a subchapter for the period.
```{r}
...
Recode with final visit's diagnosis in sequence
```{r}
source("../src/functions.r")
