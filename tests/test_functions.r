...
test_that("convert_visits_to_sequences", {
  ...
  df_convert_seq_to_visits_out <- convert_visits_to_sequences(df_convert_seq_to_visits_in,
    uid_col = patient_clinic_uid,
    date_col = date
  )

  expect_equal(df_convert_seq_to_visits_out$patient_clinic_uid,
    c("1", "2", "3"),
    tolerance = 1e-4
  )
...
test_that("convert_visits_to_sequences", {
  df_count_in <- tibble::tribble(
    ~patient_clinic_uid, ~date, ~sequence_start_date, ~sequence_end_date,
...
### Test add_patient_clinic_uid
test_dfs$add_patient_clinic_uid <- tibble::tribble(
...
test_dfs$constructed_sequences <- test_dfs$add_patient_clinic_uid %>%
  add_patient_clinic_uid(
    clinic_id_col = clinic_id,
    patient_id_col = patient_id
  )

test_that("add_patient_clinic_uid", {
  ...
  expect_equal(
    test_dfs$constructed_sequences[["patient_clinic_uid"]],
    c("11", "11", "11", "12", "12", "23"),
    fixed = TRUE)
  # Testing column names
  expect_equal(
    names(test_dfs$constructed_sequences),
    c("clinic_id", "patient_id", "adiagnosekode", "datotid_start", "patient_clinic_uid"),
    fixed = TRUE)
  # Testing column classes
  expect_equal(
...
test_dfs$add_patient_clinic_uid_no_threshold <- tibble::tribble(
  ~clinic_id, ~patient_id, ~adiagnosekode, ~datotid_start,
  1, 1, "F0", "2013-07-23 00:00:00",
  1, 1, "F1", "2013-07-24 00:00:00",
  1, 1, "F9", "2013-09-25 00:00:00",
...
test_dfs$constructed_sequences_no_threshold <- test_dfs$add_patient_clinic_uid %>%
  add_patient_clinic_uid(
    clinic_id_col = clinic_id,
    patient_id_col = patient_id
  )

test_that("add_patient_clinic_uid without threshold_months", {
  ...
  expect_equal(
    test_dfs$constructed_sequences_no_threshold[["patient_clinic_uid"]],
    c("11", "11", "11", "12", "12", "23"),
    fixed = TRUE
  )
  ## Finished testing 'test_dfs$constructed_sequences'                        ####
